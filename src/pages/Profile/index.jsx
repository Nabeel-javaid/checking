...
const etherscanApiKey = "UXWX37Y2Y4PQM8ZZ1XQ2KBWH6AVZ8J7RYU";
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `https://api-goerli.etherscan.io/api?module=account&action=txlist&address=${walletAddress}&startblock=0&endblock=99999999&sort=desc&apikey=${etherscanApiKey}`
        );
        const data = await response.json();

        if (data.result) {
          const truncatedTransactions = data.result
            .slice(0, 3)
            .map((transaction) => ({
              ...transaction,
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            ...
            >
              ...
              >
                <strong className="ownername">Owner: {ownerAddress || walletAddress}</strong>
              </Typography>
          ...
          <Dialog open={cancelDialogOpen} onClose={handleCancelCancel}>
            ...
            <DialogContent>
              <p>
                Market Closing is irreversible and trading cannot happen in
                closed markets, do you still want to close your market?
              </p>
            </DialogContent>
            <DialogActions>
